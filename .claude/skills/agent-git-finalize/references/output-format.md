...
| ⚠️ | Completed with intervention (fallback used, conflict resolved) |
| 🔁 | Retried — show count as `🔁×N` |
| ℹ️ | Informational notice (tracker reminder, etc.) |
...
| 1.5 | Typecheck gate             | ✅ | `pnpm -w typecheck` passed |
...
```markdown
| 1.5 | Typecheck gate             | ❌ | `pnpm -w typecheck` failed in `<package>` — commit blocked |
...
| 6   | Merged to main             | ❌ | Conflicts could not be resolved — manual intervention required |
